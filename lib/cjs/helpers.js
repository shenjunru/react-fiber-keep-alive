...
const findChildHostFibers = (fiber) => {
    ...
    while (stack.length) {
        const current = stack.pop();
        if (!current || current.tag === exports.FiberTag.HostPortal) {
            continue;
        }
...
const replaceFiber = (oldFiber, newFiber) => {
    ...
    const portalFibers = (0, exports.findFibers)(newFiber, (node) => node.tag === exports.FiberTag.HostPortal);
    portalFibers.forEach((portalFiber) => {
        var _a;
        const containerInfo = (_a = portalFiber.stateNode) === null || _a === void 0 ? void 0 : _a.containerInfo;
        containerInfo && (0, exports.findChildHostFibers)(portalFiber.child).forEach((fiber) => {
            const thisHostNode = getHostNode(fiber);
